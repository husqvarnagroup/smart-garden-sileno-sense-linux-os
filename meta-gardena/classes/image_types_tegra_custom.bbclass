...
DEPENDS += "genimage-native u-boot-tools-native mtools-native"
...
INITRD_FLASHER_DTBFILE ?= "${@os.path.basename(d.getVar('INITRD_FLASHER_KERNEL_DEVICETREE')) if d.getVar('INITRD_FLASHER_KERNEL_DEVICETREE') else ''}"
LNXFILE ?= "boot.img"
LNXSIZE ?= "83886080"
...
tegraflash_create_flash_config:tegra210() {
    ...
    bbwarn "Overridden file"
    [ -n "$infile" ] || infile="${TOPDIR}/../meta-gardena/flash-cfg/${PARTITION_LAYOUT_TEMPLATE}"
    sed \
...
create_tegraflash_pkg:tegra210() {
    ...
    cp ${DEPLOY_DIR_IMAGE}/uboot-env-${MACHINE}.bin uboot-env.bin
    cp ${DEPLOY_DIR_IMAGE}/boot-${MACHINE}.scr boot.scr

    rm -f doflash.sh
    cat > doflash.sh <<END
#!/bin/sh
MACHINE=${MACHINE} ./tegra210-flash-helper.sh --datafile overlay.ext4 $DATAARGS flash.xml.in ${DTBFILE} ${MACHINE}.cfg ${ODMDATA} ${LNXFILE} ${IMAGE_BASENAME}.${IMAGE_TEGRAFLASH_FS_TYPE} "\$@"
END
    chmod +x doflash.sh
    if [ -n "${IMAGE_TEGRAFLASH_INITRD_FLASHER}" ]; then
...
#!/bin/sh
MACHINE=${MACHINE} ./tegra210-flash-helper.sh -c "blowfuses odmfuse_pkc.xml" --no-flash $DATAARGS flash.xml.in ${DTBFILE} ${MACHINE}.cfg ${ODMDATA} ${LNXFILE} ${IMAGE_BASENAME}.${IMAGE_TEGRAFLASH_FS_TYPE} "\$@"
END
    ...
    if [ "${TEGRA_SPIFLASH_BOOT}" = "1" ]; then
        ...
        cat > dosdcard.sh <<END
#!/bin/sh
MACHINE=${MACHINE} BOARDID=\${BOARDID:-${TEGRA_BOARDID}} FAB=\${FAB:-${TEGRA_FAB}} ./tegra210-flash-helper.sh --sdcard -s ${TEGRAFLASH_SDCARD_SIZE} -b ${IMAGE_BASENAME} $DATAARGS flash.xml.in ${DTBFILE} ${MACHINE}.cfg ${ODMDATA} ${LNXFILE} ${IMAGE_BASENAME}.${IMAGE_TEGRAFLASH_FS_TYPE} "\$@"
END
        chmod +x dosdcard.sh
    fi
    tegraflash_custom_post
...
do_image_tegraflash[depends] += "${TEGRAFLASH_PKG_DEPENDS} dtc-native:do_populate_sysroot coreutils-native:do_populate_sysroot \
                                 ...
                                 u-boot-env-generator:do_deploy"
