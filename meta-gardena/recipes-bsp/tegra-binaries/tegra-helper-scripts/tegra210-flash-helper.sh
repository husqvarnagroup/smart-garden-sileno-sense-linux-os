...
if [ $bup_blob -ne 0 ]; then
    ...
    verityfile_sed="-es,VERITYFILE,${imgfile}.verity,"
    libfsfile_sed="-es,LIBFSFILE,${libfile},"
else
...
sed -e"s,VERFILE,${MACHINE}_bootblob_ver.txt," -e"s,DTBFILE,$DTBFILE," \
    -e"s,TBCFILE,$TBCFILENAME," $appfile_sed $verityfile_sed $libfsfile_sed flash.xml.tmp > flash.xml
